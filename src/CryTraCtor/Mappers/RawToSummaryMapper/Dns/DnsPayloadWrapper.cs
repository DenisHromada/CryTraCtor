...
public class DnsPayloadWrapper
{
    ...
    public DnsResourceRecord GetQuery()
    {
        ...
        var parsedQuery = new DnsResourceRecord(
    ...
    public Collection<DnsResourceRecord> GetAnswers()
    {
        var answers = new Collection<DnsResourceRecord>();

        foreach (var answer in _dnsPacket.Answers)
        {
            var parsedAnswer = new DnsResourceRecord(
